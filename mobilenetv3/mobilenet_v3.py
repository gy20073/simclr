...
from resnet import filter_trainable_variables, add_to_collection
...
@slim.add_arg_scope
def mobilenet_func(depth_multiplier=1.0,
              scope='MobilenetV3',
              conv_defs=None,
              **kwargs):
  "a function form of the mobilenet function"
  if conv_defs is None:
    conv_defs = V3_LARGE
  if 'multiplier' in kwargs:
    raise ValueError('mobilenetv2 doesn\'t support generic '
  ...
  depth_args = {}
  with slim.arg_scope((lib.depth_multiplier,), **depth_args):
    def model(input_tensor, is_training):
        net, end_points = lib.mobilenet(
            input_tensor,
            num_classes=None,
            conv_defs=conv_defs,
            scope=scope,
        ...
        trainable_variables = {}
        # fill the previous 4 with empty list
        for i in range(4):
